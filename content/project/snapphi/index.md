...
title = 'Snapphi'
type = 'project'
description = 'A payment orchestration platform for the Indonesian market -- aggregates Xendit, Midtrans, and Doku behind a single API with intelligent routing, automated settlement, and a multi-tenant admin dashboard.'
image = ''
repository = ''
...
- **Multi-tenant RBAC** -- permissions operate at two scopes: system (user, admin) and merchant (owner). System admins can dynamically create custom roles with custom permission sets for both scopes. Permissions are cached in Redis and take effect immediately on change -- no JWT re-issue needed
