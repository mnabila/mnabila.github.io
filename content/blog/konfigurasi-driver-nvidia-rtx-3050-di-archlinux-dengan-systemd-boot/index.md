...
Setup driver NVIDIA di Archlinux itu bukan hal yang bisa dianggap remeh, terutama kalau pakai Wayland. Tidak seperti di Ubuntu atau Fedora yang biasanya tinggal klik "Additional Drivers", di Arch saya harus setup semuanya secara manual seperti install package yang tepat, konfigurasi kernel module, enable DRM modesetting, dan memastikan semuanya dimuat saat boot.

Situasinya makin spesifik kalau menggunakan **systemd-boot** sebagai bootloader, saya tidak bisa tinggal edit `/etc/default/grub` untuk menambahkan kernel parameter seperti pada bootloader GRUB2. Harus langsung edit boot entry yang ada di directory `/boot/loader/entries/`.
...
| Driver        | Keterangan                                                                                  |
| ------------- | ------------------------------------------------------------------------------------------- |
| `nvidia`      | Driver proprietary closed-source yang paling stabil dan mature                              |
| `nvidia-open` | Kernel module open-source dari NVIDIA yang direkomendasikan untuk arsitektur Turing ke atas |
...
Karena menggunakan kernel **linux-zen**, saya perlu install package driver yang sesuai:
...
Kenapa `nvidia-dkms` dan bukan `nvidia`? Karena saya pakai kernel **linux-zen**, bukan `linux` standar. Package `nvidia` (tanpa `-dkms`) hanya untuk kernel `linux` default. DKMS memastikan module otomatis di-compile ulang setiap kali kernel di-update.
